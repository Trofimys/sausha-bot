...
from bot.yookassa import YooKassaClient, YooKassaError
...
class BotApp:
    ...
    def create_yookassa_payment(
        self, callback: dict[str, Any], purpose: str, value: str, method: str
    ) -> None:
        message = callback.get("message")
        user = callback.get("from")
        if not message or not user:
        ...
        if resolved is None:
            return
        amount, title, description, server_code = resolved

        username = str(self.bot_info.get("username") or "")
        return_url = f"https://t.me/{username}" if username else "https://t.me"

        try:
            payment = self.yookassa.create_payment(
                amount_rub=amount,
                description=description,
                payload=f"{purpose}:{user['id']}:{value}",
        ...
        self.edit_message_content(
            message=message,
            text=build_crypto_invoice_text(title, amount),
            reply_markup=yookassa_invoice_keyboard(pay_url, payment_id, method),
            photo_path=self.settings.top_up_image_path,
        )
    ...
    def check_invoice(self, callback: dict[str, Any], invoice_id: str) -> None:
        ...
        provider = str(stored["provider"] or "cryptobot")
    ...
    def process_admin_callback(self, callback: dict[str, Any], data: str) -> None:
        ...
        if data in {"admin:account", "admin:refresh"}:
            ...
            self.answer_callback(callback)
            return
        elif data.startswith("admin:servers:price:"):
            server_code = data.removeprefix("admin:servers:price:")
            server_item = get_server_map().get(server_code)
            if server_item is None:
                self.answer_callback(callback, "Сервер не найден.", show_alert=True)
                return
            ...
            self.telegram.send_message(
                chat_id=self._chat_id(message),
                text=build_server_price_prompt_text(
                    server_item["name"], float(server_item.get("price_rub", 0) or 0)
                ),
            )
            return
    ...
    def process_admin_price_input(self, message: dict[str, Any], text: str) -> None:
        ...
        server_code = self.pending_admin_price_servers.pop(int(user["id"]), None)
        ...
        if server_item is None:
            self.telegram.send_message(
                chat_id=self._chat_id(message), text="Сервер не найден."
            )
            return

        value_text = text.strip().replace(",", ".").replace("₽", "").strip()
        try:
        ...
        if updated is None:
            self.telegram.send_message(
                chat_id=self._chat_id(message), text="Не удалось сохранить цену."
            )
            return
...
class _HealthHandler(BaseHTTPRequestHandler):
    ...
    def log_message(self, *args: Any) -> None:  # глушим шумный access-лог
        return
...
def start_health_server() -> None:
    ...
    try:
        ...
        logging.warning("PORT=%r не число, health-сервер не запущен", port_raw)
        return
...
def start_keep_alive(interval_seconds: int = 600) -> None:
    ...
    thread.start()
    logging.info("Keep-alive enabled: %s каждые %s c", url, interval_seconds)
