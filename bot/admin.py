...
def yookassa_invoice_keyboard(pay_url: str, invoice_id: str, method: str) -> dict[str, Any]:
    button_text = "Оплатить по СБП" if method == "sbp" else "Оплатить картой"
    return {
        "inline_keyboard": [
            [
                {"text": button_text, "url": pay_url},
            ],
            [
                {"text": "Проверить оплату", "callback_data": f"invoice:check:{invoice_id}"},
            ],
...
def build_servers_text() -> str:
    ...
    for item in load_server_catalog():
        ...
        price = float(item.get("price_rub", 0) or 0)
        lines.append(
...
def server_versions_keyboard(proxy6_client: Proxy6Client, server_code: str) -> dict[str, Any]:
    ...
    rows.append(
        [{"text": "💰 Изменить цену", "callback_data": f"admin:servers:price:{server_code}"}]
    )
    rows.append([{"text": "Назад", "callback_data": "admin:servers"}])
    return {"inline_keyboard": rows}


def build_server_versions_text(server_name: str, price_rub: float | None = None) -> str:
    price_line = ""
    if price_rub is not None:
        price_line = f"Цена: <code>{price_rub:.2f} ₽</code>\n"
    return (
        "<b>Настройка сервера</b>\n"
        f"Сервер: <code>{escape(server_name)}</code>\n"
        f"{price_line}\n"
        "Выберите тип прокси или измените цену."
    )


def build_server_price_prompt_text(server_name: str, current_price: float) -> str:
    return (
        "<b>Изменение цены</b>\n"
        f"Сервер: <code>{escape(server_name)}</code>\n"
        f"Текущая цена: <code>{current_price:.2f} ₽</code>\n\n"
        "Отправьте новую цену в рублях (например: <code>79</code> или <code>79.5</code>)."
    )
