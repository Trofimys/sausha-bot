...
@bot1_dp.message(lambda m: m.text and "A_ToolsX" in m.text)
async def bot1_delete_spam_private(message: Message):
    try:
        await message.delete()
        logger.info(f"[Bot1] Удалено спам-сообщение от {message.from_user.id}: {message.text[:50]}")
    except Exception as e:
        logger.warning(f"[Bot1] Не удалось удалить спам: {e}")

@bot1_dp.channel_post(lambda m: m.text and "A_ToolsX" in m.text)
async def bot1_delete_spam_channel(message: Message):
    try:
        await message.delete()
        logger.info(f"[Bot1] Удалено спам-сообщение в канале: {message.text[:50]}")
    except Exception as e:
