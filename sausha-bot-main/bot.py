...
def bot1_admin_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🚫  Заблокировать", callback_data="b1_admin_block_add"),
         InlineKeyboardButton(text="✅  Разблокировать", callback_data="b1_admin_block_remove")],
...
async def bot1_admin_block_add_input(message: Message):
    ...
    target = resolve_block_target(parts[0]) if parts else None
    if target is None:
        await message.reply(
            "❌ Не нашёл пользователя. По @username находятся только те, кто писал ботам. "
            "Попробуй числовой ID.")
        return
    bot1_admin_state.pop(message.from_user.id, None)
    if target == ADMIN_ID:
        await message.reply("⚠️ Нельзя заблокировать администратора.")
        return
...
async def bot1_admin_block_remove_input(message: Message):
    ...
    if target is None:
        await message.reply("❌ Не нашёл пользователя. Попробуй числовой ID.")
        return
    bot1_admin_state.pop(message.from_user.id, None)
...
@bot1_dp.message(lambda m: _is_spam(m))
async def bot1_delete_spam_private(message: Message):
    txt = message.text or message.caption or ""
    try:
        await message.delete()
        logger.info(f"[Bot1] Удалено спам-сообщение от {message.from_user.id if message.from_user else '?'}: {txt[:50]}")
    except Exception as e:
        logger.warning(f"[Bot1] Не удалось удалить спам: {e}")

@bot1_dp.channel_post(lambda m: _is_spam(m))
async def bot1_delete_spam_channel(message: Message):
    txt = message.text or message.caption or ""
    try:
        await message.delete()
...
async def bot1_send_sub_required(message: Message, bot_username: str, param: str):
    ...
    await message.answer(
        "❗ Чтобы оставить анонимный комментарий, нужно быть участником нашего чата.\n\n"
        "1️⃣ Подпишись на чат по кнопке ниже\n"
...
def _start_keepalive_http():
    ...
    port = int(os.environ.get("SAUSHA_HTTP_PORT", "10000"))
    try:
        HTTPServer(("0.0.0.0", port), Handler).serve_forever()
    except OSError as e:
        logger.warning("[Bot2] keep-alive HTTP не поднят на :%s (%s) — не критично", port, e)
...
def block_user(uid: int, reason: str = "") -> None:
    blocked_users[str(uid)] = {"reason": reason, "at": datetime.now().isoformat()}
    _save_json(BLOCKED_FILE, blocked_users)
...
def find_uid_by_username(username: str) -> int | None:
    ...
    if not target:
        return None
    pools = [start_logs, message_logs, anon_messages_log]
    for pool in pools:
        for e in pool:
            uname = (e.get("username") or "").strip().lower()
...
def resolve_block_target(raw: str) -> int | None:
    ...
    if raw.lstrip("-").isdigit():
        return int(raw)
    return find_uid_by_username(raw)

def build_blocked_list_text() -> str:
    if not blocked_users:
        return "🚫 Заблокированные\n\nСписок пуст."
    lines = ["🚫 Заблокированные\n"]
    for uid, info in list(blocked_users.items())[:50]:
        reason = (info or {}).get("reason") or ""
        reason_txt = f" — {reason}" if reason else ""
        lines.append(f"• {uid}{reason_txt}")
    return "\n".join(lines)
...
async def post_blocked_to_log_channel(context, update, ctype, ctext, blocked_reason) -> int | None:
    ...
    lines = [
        ...
        "┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄",
        f"📎 Тип: {ctype}",
        f"❌ Причина блока: {blocked_reason}",
    ]
    if ctext:
        safe_text = ctext[:300].replace("`", "'")
        lines.append(f"💬 Текст:\n`{safe_text}`")
...
async def bot2_cmd_cancel(update: Update, context: PTBContextTypes.DEFAULT_TYPE):
    ...
    popped = (context.user_data.pop("awaiting_broadcast", None)
              ...
              or context.user_data.pop("awaiting_block_add", None)
...
async def admin_callback(update: Update, context: PTBContextTypes.DEFAULT_TYPE):
    ...
    elif data == "admin_block_add":
        context.user_data["awaiting_block_add"] = True
        await query.edit_message_text(
            "🚫 *Блокировка*\n\n"
            "Отправь числовой ID или @username, при желании через пробел причину.\n\n"
            "Примеры:\n`123456789 спам`\n`@username`\n\n/cancel — отмена",
    ...
    elif data == "admin_block_remove":
        context.user_data["awaiting_block_remove"] = True
        await query.edit_message_text(
            "✅ *Разблокировка*\n\n"
            "Отправь числовой ID или @username.\n\n/cancel — отмена",
            parse_mode="Markdown",
...
async def bot2_handle_message(update: Update, context: PTBContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("awaiting_block_add") and uid == ADMIN_ID:
        ...
        if target is None:
            await update.message.reply_text(
                "❌ Не нашёл пользователя. По @username можно найти только тех, "
                "кто уже писал ботам. Попробуй числовой ID.")
            return
        context.user_data.pop("awaiting_block_add", None)
        if target == ADMIN_ID:
        ...
        block_user(target, reason)
        rtxt = f"\nПричина: {reason}" if reason else ""
        await update.message.reply_text(f"🚫 Заблокирован: `{target}`{rtxt}",
                                        parse_mode="Markdown", reply_markup=admin_keyboard())
        return

    if context.user_data.get("awaiting_block_remove") and uid == ADMIN_ID:
        raw = (update.message.text or "").strip()
        target = resolve_block_target(raw)
        if target is None:
            await update.message.reply_text(
                "❌ Не нашёл пользователя. Попробуй числовой ID.")
            return
        context.user_data.pop("awaiting_block_remove", None)
        ok = unblock_user(target)
        msg = f"✅ Разблокирован: `{target}`" if ok else f"ℹ️ `{target}` не был заблокирован."
